"""Run the PaSST candidate in the IDEA-049 MeowAgeNet screening protocol.

This runner owns PaSST feature extraction and candidate-specific audit logs. It
...
RECIPE_PATH = (
    REPO_ROOT / "configs" / "experiment" / "idea049" / "passt_s_ap476_frozen_v1.json"
)
...
TORCH_CACHE = REPO_ROOT / "data" / "models" / "torch"
ENGINE_PATH = REPO_ROOT / "scripts" / "run_meowagenet_idea049.py"
PIPELINE_ID = "passt_s_ap476_frozen_mlp"
MINIMUM_AUDIO_SAMPLES = 5_120
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-subdir", default="idea049_passt_s_ap476_v1")
...
def verify_inputs(protocol: dict[str, Any], recipe: dict[str, Any]) -> None:
    ...
    if recipe["pipeline_id"] != PIPELINE_ID:
        raise RuntimeError("Unsupported PaSST IDEA-049 pipeline")
...
def checkpoint_path(recipe: dict[str, Any]) -> Path:
    model = recipe["model"]
    path = TORCH_CACHE / "hub" / "checkpoints" / model["checkpoint_file"]
    if not path.is_file():
        raise FileNotFoundError(
            f"Download the audited PaSST release asset before extraction: {path}"
        )
    if path.stat().st_size != int(model["checkpoint_bytes"]):
        raise RuntimeError("PaSST checkpoint size mismatch")
    if sha256(path) != model["checkpoint_sha256"]:
        raise RuntimeError("PaSST checkpoint SHA-256 mismatch")
    return path


def feature_paths(run_root: Path) -> tuple[Path, Path]:
    root = run_root / "features"
    return root / "passt_call_embeddings.npz", root / "feature_manifest.json"


def load_passt_model(recipe: dict[str, Any], device: torch.device) -> torch.nn.Module:
    if importlib.metadata.version("hear21passt") != recipe["model"]["runtime_package_version"]:
        raise RuntimeError("hear21passt package version differs from the recipe")
    ...
    try:
        ...
        model = get_basic_model(
            mode=recipe["model"]["wrapper_mode"],
            arch=recipe["model"]["runtime_arch"],
        )
    finally:
    ...
    if model.mode != "embed_only" or int(model.sample_rate) != 32_000:
        raise RuntimeError("Unexpected PaSST wrapper configuration")
    if int(model.net.num_features) != int(recipe["model"]["hidden_size"]):
        raise RuntimeError("Unexpected PaSST embedding dimension")
    patchout = (
        int(model.net.u_patchout),
        int(model.net.s_patchout_t),
        int(model.net.s_patchout_f),
    )
    if patchout != (0, 0, 0):
        raise RuntimeError(f"Inference Patchout must be disabled, observed {patchout}")
    return model
...
def prepare_embeddings(
    ...
    if batch_size != 1:
        raise ValueError("PaSST variable-duration extraction is locked to batch size 1")
    ...
    if feature_path.is_file():
        ...
        if loaded["embeddings"].shape != (792, 768):
            raise RuntimeError("Existing PaSST embedding cache has the wrong shape")
        if not manifest_path.is_file() or sha256(feature_path) != read_json(manifest_path)["feature_sha256"]:
            raise RuntimeError("Existing PaSST feature cache failed checksum verification")
    ...
    model = load_passt_model(recipe, device)
    ...
    with warnings.catch_warnings():
        warnings.filterwarnings(
            "ignore", message="stft with return_complex=False is deprecated.*", category=UserWarning
        )
        warnings.filterwarnings(
            "ignore", message="Input image size .* doesn't match model .*", category=UserWarning
        )
        for call_index, row in enumerate(rows.itertuples(index=False), start=1):
            ...
            if tuple(embedding.shape) != (1, 768) or not torch.isfinite(embedding).all():
                raise RuntimeError(f"Invalid PaSST embedding at call {call_index - 1}")
            embeddings.append(embedding.float().cpu().numpy())
            if call_index % 25 == 0 or call_index == len(rows):
                print(f"PaSST embeddings {call_index}/792", flush=True)
    ...
    if embedding_array.shape != (792, 768) or not np.isfinite(embedding_array).all():
        raise RuntimeError("Invalid PaSST embedding output")
    if padded_calls != int(recipe["preprocessing"]["clips_below_160ms"]):
    ...
    manifest = {
        ...
        "inference_revision": recipe["model"]["inference_revision"],
        "runtime_package": f"hear21passt=={importlib.metadata.version('hear21passt')}",
        "pooling": recipe["model"]["pooling"],
        "wrapper_mode": recipe["model"]["wrapper_mode"],
        "inference_patchout": recipe["model"]["inference_patchout"],
        "source_sample_rates_hz": sorted(source_sample_rates),
        "short_calls_padded_to_160ms": padded_calls,
        ...
        "backbone_parameters": sum(parameter.numel() for parameter in model.net.parameters()),
...
def load_feature_store(run_root: Path, recipe: dict[str, Any]) -> engine.FeatureStore:
    ...
    if sha256(feature_path) != manifest["feature_sha256"]:
        raise RuntimeError("PaSST feature-cache checksum mismatch")
    loaded = np.load(feature_path)
    ...
    if len(np.unique(store.cat_ids)) != 111 or not np.isfinite(store.embeddings).all():
        raise RuntimeError("Invalid PaSST feature store")
    return store
...
def stage_manifest(
    ...
    return {
        ...
        "hear21passt": importlib.metadata.version("hear21passt"),
...
def aggregate_initial(
    ...
    for repeat in stage["repeats"]:
        ...
        for outer_fold in stage["outer_folds"]:
            ...
            if not path.is_file():
                raise RuntimeError(f"Missing PaSST prediction file: {path}")
        ...
        if len(calls) != 792 or calls["call_index"].nunique() != 792:
            raise RuntimeError("PaSST complete OOF must contain 792 unique calls")
        if len(animals) != 111 or animals["cat_id"].nunique() != 111:
            raise RuntimeError("PaSST complete OOF must contain 111 cats")
...
def run_initial(
    ...
    if manifest_path.exists():
        ...
        if any(existing.get(key) != manifest.get(key) for key in checked):
            raise RuntimeError("PaSST initial-screening resume manifest differs")
        if not resume:
            raise FileExistsError("PaSST initial run exists; pass --resume after review")
    else:
        write_json(manifest_path, manifest)
    ...
    for repeat in stage["repeats"]:
        for outer_fold in stage["outer_folds"]:
            for base_seed in stage["base_seeds"]:
                ...
                print(
                    f"=== PaSST repeat={repeat} fold={outer_fold} base_seed={base_seed} "
                    f"full_seed={seed} ===",
                    flush=True,
                )
...
def main() -> None:
    ...
    print(
        f"IDEA-049 PaSST stage={args.stage}; device={device}; "
