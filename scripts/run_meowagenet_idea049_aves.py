"""Run the AVES-base-bio candidate in the IDEA-049 MeowAgeNet screening protocol.

This runner owns AVES feature extraction and candidate-specific audit logs. It
...
RECIPE_PATH = (
    REPO_ROOT / "configs" / "experiment" / "idea049" / "aves_base_bio_frozen_v1.json"
)
...
MODEL_ROOT = REPO_ROOT / "data" / "models" / "aves"
ENGINE_PATH = REPO_ROOT / "scripts" / "run_meowagenet_idea049.py"
PIPELINE_ID = "aves_base_bio_frozen_mlp"
MINIMUM_AUDIO_SAMPLES = 400


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser()
    parser.add_argument("--stage", choices=("prepare", "smoke", "initial"), required=True)
    parser.add_argument("--output-subdir", default="idea049_aves_base_bio_v1")
...
def verify_inputs(protocol: dict[str, Any], recipe: dict[str, Any]) -> None:
    ...
    if recipe["pipeline_id"] != PIPELINE_ID:
        raise RuntimeError("Unsupported AVES IDEA-049 pipeline")
...
def model_paths(recipe: dict[str, Any]) -> tuple[Path, Path]:
    model = recipe["model"]
    checkpoint = MODEL_ROOT / model["checkpoint_file"]
    config = MODEL_ROOT / model["config_file"]
    if not checkpoint.is_file():
        raise FileNotFoundError(
            f"Download the audited AVES checkpoint before extraction: {checkpoint}"
        )
    if checkpoint.stat().st_size != int(model["checkpoint_bytes"]):
        raise RuntimeError("AVES checkpoint size mismatch")
    if sha256(checkpoint) != model["checkpoint_sha256"]:
        raise RuntimeError("AVES checkpoint SHA-256 mismatch")
    if not config.is_file() or config.stat().st_size != int(model["config_bytes"]):
        raise RuntimeError("AVES model-config size mismatch")
    if sha256(config) != model["config_sha256"]:
        raise RuntimeError("AVES model-config SHA-256 mismatch")
    return checkpoint, config


def feature_paths(run_root: Path) -> tuple[Path, Path]:
    root = run_root / "features"
    return root / "aves_call_embeddings.npz", root / "feature_manifest.json"


def load_aves_model(recipe: dict[str, Any], device: torch.device) -> torch.nn.Module:
    if importlib.metadata.version("esp-aves") != recipe["model"]["runtime_package_version"]:
        raise RuntimeError("esp-aves package version differs from the recipe")
    checkpoint, config = model_paths(recipe)
    ...
    if sum(parameter.numel() for parameter in model.parameters()) != 94_370_944:
        raise RuntimeError("Unexpected AVES-base-bio parameter count")
    return model
...
def prepare_embeddings(
    ...
    if batch_size != 1:
        raise ValueError("AVES variable-duration extraction is locked to batch size 1")
    ...
    with warnings.catch_warnings():
        warnings.filterwarnings(
            "ignore", message="1Torch was not compiled with flash attention.*", category=UserWarning
        )
        for call_index, row in enumerate(rows.itertuples(index=False), start=1):
            ...
            if samples_before_padding < MINIMUM_AUDIO_SAMPLES:
                waveform = torch.nn.functional.pad(waveform, (0, MINIMUM_AUDIO_SAMPLES - samples_before_padding))
                padded_calls += 1
            waveform = waveform.unsqueeze(0).to(device=device, dtype=torch.float32)
            with torch.inference_mode():
                frame_features = model.extract_features(waveform, layers=-1)
                embedding = frame_features.mean(dim=1)
            frame_counts.append(int(frame_features.shape[1]))
            if tuple(frame_features.shape[::2]) != (1, 768):
                raise RuntimeError(f"Invalid AVES frame output at call {call_index - 1}")
            if tuple(embedding.shape) != (1, 768) or not torch.isfinite(embedding).all():
                raise RuntimeError(f"Invalid AVES embedding at call {call_index - 1}")
            embeddings.append(embedding.float().cpu().numpy())
            if call_index % 25 == 0 or call_index == len(rows):
                print(f"AVES embeddings {call_index}/792", flush=True)
    ...
    if embedding_array.shape != (792, 768) or not np.isfinite(embedding_array).all():
        raise RuntimeError("Invalid AVES embedding output")
    if padded_calls != int(recipe["preprocessing"]["clips_below_25ms"]):
    ...
    np.savez_compressed(
        ...
        samples_at_16khz=np.asarray(sample_counts, dtype=np.int64),
        temporal_frames=np.asarray(frame_counts, dtype=np.int64),
        padded_to_25ms=np.asarray(
    ...
    manifest = {
        ...
        "official_revision": recipe["model"]["official_revision"],
        "runtime_package": f"esp-aves=={importlib.metadata.version('esp-aves')}",
        "pooling": recipe["model"]["pooling"],
        "source_sample_rates_hz": sorted(source_sample_rates),
        "short_calls_padded_to_25ms": padded_calls,
        "temporal_frames": {
            "minimum": int(np.min(frame_counts)),
            "median": float(np.median(frame_counts)),
            "maximum": int(np.max(frame_counts)),
        },
...
def load_feature_store(run_root: Path, recipe: dict[str, Any]) -> engine.FeatureStore:
    ...
    if sha256(feature_path) != manifest["feature_sha256"]:
        raise RuntimeError("AVES feature-cache checksum mismatch")
    loaded = np.load(feature_path)
    ...
    if len(np.unique(store.cat_ids)) != 111 or not np.isfinite(store.embeddings).all():
        raise RuntimeError("Invalid AVES feature store")
    return store
...
def stage_manifest(
    ...
    return {
        ...
        "esp_aves": importlib.metadata.version("esp-aves"),
...
def aggregate_initial(
    ...
    for repeat in stage["repeats"]:
        ...
        for outer_fold in stage["outer_folds"]:
            ...
            if not path.is_file():
                raise RuntimeError(f"Missing AVES prediction file: {path}")
        ...
        if len(calls) != 792 or calls["call_index"].nunique() != 792:
            raise RuntimeError("AVES complete OOF must contain 792 unique calls")
        if len(animals) != 111 or animals["cat_id"].nunique() != 111:
            raise RuntimeError("AVES complete OOF must contain 111 cats")
...
def run_initial(
    ...
    if manifest_path.exists():
        ...
        if any(existing.get(key) != manifest.get(key) for key in checked):
            raise RuntimeError("AVES initial-screening resume manifest differs")
        if not resume:
            raise FileExistsError("AVES initial run exists; pass --resume after review")
    else:
        write_json(manifest_path, manifest)
    ...
    for repeat in stage["repeats"]:
        for outer_fold in stage["outer_folds"]:
            for base_seed in stage["base_seeds"]:
                ...
                print(
                    f"=== AVES repeat={repeat} fold={outer_fold} base_seed={base_seed} "
                    f"full_seed={seed} ===",
                    flush=True,
                )
...
def main() -> None:
    ...
    print(
        f"IDEA-049 AVES-base-bio stage={args.stage}; device={device}; "
