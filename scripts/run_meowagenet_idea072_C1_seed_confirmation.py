"""Run IDEA-072 independent-seed confirmation of bounded additive C1."""
...
PROTOCOL_PATH = (
    ...
    / "configs"
    / "protocol"
    / "meowagenet_idea072_C1_seed_confirmation_v1.json"
)
PIPELINES = (
    "A0_ast_only",
    "A1_age_residual",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-subdir", default="meowagenet_idea072_C1_seed_confirmation_v1"
    )
...
def initial_logit_differences(
    ...
    if parameters != EXPECTED_PARAMETERS:
        raise RuntimeError(f"IDEA-072 parameter audit mismatch: {parameters}")
    differences = {
        pipeline: float(np.max(np.abs(logits[pipeline] - logits[PIPELINES[0]])))
        for pipeline in PIPELINES[1:]
    }
    if any(value != 0.0 for value in differences.values()):
        raise RuntimeError("IDEA-072 pipelines are not identical at initialization")
    return differences


def verify_protocol(protocol: dict[str, Any]) -> None:
    if protocol.get("protocol_id") != "meowagenet-idea072-C1-seed-confirmation-v1":
        raise RuntimeError("Unexpected IDEA-072 protocol")
    if protocol.get("status") != "locked_before_initial_evaluation":
        raise RuntimeError("IDEA-072 protocol is not locked")
    model = protocol["model"]
    if tuple(model["pipelines"]) != PIPELINES:
        raise RuntimeError("IDEA-072 pipeline matrix changed")
    if tuple(model["base_seeds"]) != BASE_SEEDS:
        raise RuntimeError("IDEA-072 base-seed bank changed")
    if model["repeats"] != [0, 1, 2] or model["folds"] != [0, 1, 2, 3]:
        raise RuntimeError("IDEA-072 split scope changed")
    if model.get("outer_test_predictions") is not False:
        raise RuntimeError("IDEA-072 must not access outer-test predictions")
    if float(model["cap"]) != idea071.CAP:
        raise RuntimeError("IDEA-072 cap changed")
    if float(model["rms_epsilon"]) != idea071.RMS_EPSILON:
        raise RuntimeError("IDEA-072 RMS epsilon changed")
    if int(model["wide_hidden_units"]) != idea071.CAPACITY_CONTROL_HIDDEN_UNITS:
        raise RuntimeError("IDEA-072 width changed")
    if model["trainable_parameters"] != EXPECTED_PARAMETERS:
        raise RuntimeError("IDEA-072 parameter lock changed")
    expected_fits = len(PIPELINES) * len(BASE_SEEDS) * 3 * 4
    if expected_fits != 144 or int(model["total_fits"]) != expected_fits:
        raise RuntimeError("IDEA-072 fit budget is inconsistent")
    full_seeds = {
    ...
    if len(full_seeds) != 36:
        raise RuntimeError("IDEA-072 derived full seeds are not unique")
    source_protocol = read_json(idea068.PROTOCOL_PATH)
    if protocol["fixed_training"] != source_protocol["fixed_training"]:
        raise RuntimeError("IDEA-072 changed the locked IDEA-068 training recipe")
    if protocol["determinism"] != source_protocol["determinism"]:
        raise RuntimeError("IDEA-072 changed the locked IDEA-068 determinism recipe")
    ...
    for path, expected_sha in checks.items():
        if not path.is_file() or idea068.sha256(path) != expected_sha:
            raise RuntimeError(f"IDEA-072 dependency checksum mismatch: {path}")
...
def validate_completed_fit(
    ...
    for key, value in expected.items():
        if fit.get(key) != value:
            raise RuntimeError(f"IDEA-072 resume identity mismatch for {key}")
    if any(value != 0.0 for value in fit["initial_logit_differences"].values()):
        raise RuntimeError("IDEA-072 resume initial-logit audit mismatch")
    for prefix in ("validation_animal", "validation_call"):
        ...
        if not path.is_file() or idea068.sha256(path) != fit[f"{prefix}_sha256"]:
            raise RuntimeError(f"IDEA-072 resume prediction hash mismatch: {path}")
...
def add_pipeline_metrics(row: dict[str, Any], bundles: dict[str, Any]) -> None:
    ...
    for control in PIPELINES[:3]:
        ...
        row[f"C1_minus_{short}_macro_f1"] = (
...
def aggregate(fits: list[dict[str, Any]], protocol: dict[str, Any]) -> dict[str, Any]:
    ...
    split_columns = [
        ...
        "C1_minus_A1_macro_f1",
        "C1_minus_U1_macro_f1",
    ]
    split_cells = (
    ...
    for seed in model["base_seeds"]:
        ...
        per_seed_senior[str(seed)] = {
            ...
            "C1_minus_A1": float(
                senior_recalls[PIPELINES[3]] - senior_recalls[PIPELINES[1]]
            ),
        }
    delta_a0 = seed_repeats["C1_minus_A0_macro_f1"]
    delta_a1 = seed_repeats["C1_minus_A1_macro_f1"]
    ...
    conditions = {
        ...
        "every_base_seed_mean_C1_minus_A0_positive": all(
        ...
        "mean_C1_minus_A1_noninferior": float(delta_a1.mean())
        >= float(gate["minimum_mean_seed_repeat_C1_minus_A1"]),
        "base_seed_C1_minus_A1_nonnegative": sum(
            values["C1_minus_A1_macro_f1"] >= 0.0
            for values in per_seed_deltas.values()
        )
        >= int(gate["minimum_nonnegative_base_seeds_C1_minus_A1"]),
        "mean_cross_entropy_vs_A1": mean_cross_entropy[PIPELINES[3]]
        <= mean_cross_entropy[PIPELINES[1]],
        "mean_brier_vs_A1": mean_brier[PIPELINES[3]]
        <= mean_brier[PIPELINES[1]],
        "per_base_seed_senior_recall_vs_A1": all(
            value["C1_minus_A1"]
            >= float(gate["minimum_per_base_seed_senior_recall_delta_vs_A1"])
            for value in per_seed_senior.values()
        ),
        "mean_C1_minus_U1_minimum_effect": float(delta_u1.mean())
        >= float(gate["minimum_mean_seed_repeat_C1_minus_U1"]),
        "base_seed_C1_minus_U1_nonnegative": sum(
            values["C1_minus_U1_macro_f1"] >= 0.0
            for values in per_seed_deltas.values()
        )
        >= int(gate["minimum_nonnegative_base_seeds_C1_minus_U1"]),
    ...
    replacement_names = list(conditions)[8:13]
    mechanism_names = list(conditions)[13:]
    pipeline_means = {
        pipeline: float(seed_repeats[f"{pipeline}_macro_f1"].mean())
        for pipeline in PIPELINES
    }
    comparison_columns = split_columns + [
        "A1_minus_A0_macro_f1",
        "U1_minus_A1_macro_f1",
    ]
    comparisons = {}
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if len(store.call_ids) != 792 or len(np.unique(store.cat_ids.astype(str))) != 111:
        raise RuntimeError("IDEA-072 expected exactly 792 calls from 111 cats")
    ...
    if manifest_path.exists():
        if not args.resume or read_json(manifest_path) != manifest:
            raise RuntimeError("Existing IDEA-072 run manifest differs")
    else:
                ...
                for pipeline in PIPELINES:
                    ...
                    if summary_path.exists():
                        ...
                        if audit["model"]["trainable_parameters"] != expected_parameters:
                            raise RuntimeError("IDEA-072 trained parameter audit mismatch")
                ...
                for epoch in range(common_epochs):
                    ...
                    for candidate in quartet[1:]:
                        ...
                        if (
                            ...
                            raise RuntimeError("IDEA-072 paired batch order differs")
